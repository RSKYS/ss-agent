	...
	if [[ -d "/etc/ss-agent" ]]; then
		...
		if [[ -d "/etc/ss-agent/v2ray" && -f "/etc/ss-agent/v2ray/v2ray" && -f "/etc/ss-agent/v2ray/v2ctl" ]]; then
			if [[ -d "/etc/ss-agent/v2ray/conf" && -f "/etc/ss-agent/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
				...
				if grep </etc/ss-agent/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
					# 不带XTLS的v2ray-core
					coreInstallType=2
					ctlPath=/etc/ss-agent/v2ray/v2ctl
				elif grep </etc/ss-agent/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "xtls"'; then
					# 带XTLS的v2ray-core
					ctlPath=/etc/ss-agent/v2ray/v2ctl
					coreInstallType=3
				fi
		...
		if [[ -d "/etc/ss-agent/hysteria" && -f "/etc/ss-agent/hysteria/hysteria" ]]; then
			# 这里检测 hysteria
			if [[ -d "/etc/ss-agent/hysteria/conf" ]] && [[ -f "/etc/ss-agent/hysteria/conf/config.json" ]] && [[ -f "/etc/ss-agent/hysteria/conf/client_network.json" ]]; then
				hysteriaConfigPath=/etc/ss-agent/hysteria/conf/
			fi
		fi
...
mkdirTools() {
	...
	mkdir -p /etc/ss-agent/v2ray/tmp
	mkdir -p /etc/ss-agent/xray/conf
	mkdir -p /etc/ss-agent/xray/tmp
	mkdir -p /etc/ss-agent/trojan
	mkdir -p /etc/ss-agent/hysteria/conf
...
installTools() {
	...
	${upgrade} >/etc/ss-agent/install.log 2>&1
	if grep <"/etc/ss-agent/install.log" -q "changed"; then
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		curl -s https://get.acme.sh | sh >/etc/ss-agent/tls/acme.log 2>&1
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		if [[ "${historyDomainStatus}" == "y" ]]; then
			...
			echo
			echoContent yellow "请输入要配置的域名 例: www.ss-agent.com --->"
			read -r -p "域名:" domain
		fi
	else
		echo
...
switchSSLType() {
	if [[ -z "${sslType}" ]]; then
		...
		*)
		...
		touch /etc/ss-agent/tls
		echo "${sslType}" >/etc/ss-agent/tls/ssl_type
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/ss-agent/tls/acme.log >/dev/null

		local txtValue=
		txtValue=$(tail -n 10 /etc/ss-agent/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
		if [[ -n "${txtValue}" ]]; then
			echoContent green " ---> 请手动添加DNS TXT记录"
			echoContent yellow " ---> 添加方法请参考此教程，https://github.com/mack-a/ss-agent/blob/master/documents/dns_txt.md"
			...
			if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
				...
				if [[ "${txtAnswer}" == "${txtValue}" ]]; then
					...
					sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/ss-agent/tls/acme.log >/dev/null
				else
		...
		echoContent green " ---> 生成证书中"
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/ss-agent/tls/acme.log >/dev/null
	fi
}
# 自定义端口
customPortFunction() {
...
installTLS() {
	...
	if [[ -f "/etc/ss-agent/tls/${tlsDomain}.crt" && -f "/etc/ss-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/ss-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ -z $(find /etc/ss-agent/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/ss-agent/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/ss-agent/tls/${tlsDomain}.crt") ]]; then
		...
		if [[ "${installDNSACMEStatus}" == "true" ]]; then
			echo
			if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
				sudo "$HOME/.acme.sh/acme.sh" --installcert -d "*.${dnsTLSDomain}" --fullchainpath "/etc/ss-agent/tls/${tlsDomain}.crt" --keypath "/etc/ss-agent/tls/${tlsDomain}.key" --ecc >/dev/null
			fi
...
updateSELinuxHTTPPortT() {
	...
	$(find /usr/bin /usr/sbin | grep -w journalctl) -xe >/etc/ss-agent/nginx_error.log 2>&1

	if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/ss-agent/nginx_error.log | grep -q "Permission denied"; then
...
handleNginx() {
	...
	if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
		systemctl start nginx 2>/etc/ss-agent/nginx_error.log
		...
		if [[ -z $(pgrep -f nginx) ]]; then
			...
			if grep -q "journalctl -xe" </etc/ss-agent/nginx_error.log; then
				updateSELinuxHTTPPortT
			fi
...
installCronTLS() {
	...
	historyCrontab=$(sed '/ss-agent/d;/acme.sh/d' /etc/ss-agent/backup_crontab.cron)
...
renewalTLS() {
	...
	if [[ -f "/etc/ss-agent/tls/ssl_type" ]]; then
		if grep -q "buypass" <"/etc/ss-agent/tls/ssl_type"; then
			sslRenewalDays=180
		fi
	fi
	if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ "${installDNSACMEStatus}" == "true" ]]; then
		...
		if [[ ${remainingDays} -le 1 ]]; then
			...
			sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${domain}" --fullchainpath /etc/ss-agent/tls/"${domain}.crt" --keypath /etc/ss-agent/tls/"${domain}.key" --ecc
			reloadCore
			handleNginx start
		else
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		if [[ "${selectCoreType}" == "3" ]]; then
			...
			echoContent green " ---> v2ray-core版本:$(/etc/ss-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/ss-agent/hysteria/ "https://github.com/HyNetwork/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
		else
			wget -c -P /etc/ss-agent/hysteria/ "https://github.com/HyNetwork/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}" >/dev/null 2>&1
		fi
		mv "/etc/ss-agent/hysteria/${hysteriaCoreCPUVendor}" /etc/ss-agent/hysteria/hysteria
		chmod 655 /etc/ss-agent/hysteria/hysteria
	else
		echoContent green " ---> Hysteria版本:$(/etc/ss-agent/hysteria/hysteria --version | awk '{print $3}')"
		read -r -p "是否更新、升级？[y/n]:" reInstallHysteriaStatus
		if [[ "${reInstallHysteriaStatus}" == "y" ]]; then
			rm -f /etc/ss-agent/hysteria/hysteria
			installHysteria "$1"
		fi
	fi
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		echoContent green " ---> Xray-core版本:$(/etc/ss-agent/xray/xray --version | awk '{print $2}' | head -1)"
...
updateV2Ray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		echoContent green " ---> 当前v2ray-core版本:$(/etc/ss-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)"
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				if [[ "${coreInstallType}" == "2" || "${coreInstallType}" == "3" ]]; then
					...
					echoContent green " ---> 当前Xray-core版本:$(/etc/ss-agent/xray/xray --version | awk '{print $2}' | head -1)"
				fi
...
installHysteriaService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/ss-agent/hysteria/hysteria --log-level info -c /etc/ss-agent/hysteria/conf/config.json server'
...
handleV2Ray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
			...
			echoContent red "请手动执行【/etc/ss-agent/v2ray/v2ray -confdir /etc/ss-agent/v2ray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
			...
			echoContent red "请手动执行【/etc/ss-agent/hysteria/hysteria --log-level debug -c /etc/ss-agent/hysteria/conf/config.json server】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			...
			echoContent red "请手动执行【/etc/ss-agent/xray/xray -confdir /etc/ss-agent/xray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
initHysteriaNetwork() {
	...
	cat <<EOF >/etc/ss-agent/hysteria/conf/client_network.json
{
...
initHysteriaConfig() {
	...
	cat <<EOF >/etc/ss-agent/hysteria/conf/config.json
{
	"listen": ":${hysteriaPort}",
	"protocol": "${hysteriaProtocol}",
	"disable_udp": false,
	"cert": "/etc/ss-agent/tls/${currentHost}.crt",
	"key": "/etc/ss-agent/tls/${currentHost}.key",
	"auth": {
		"mode": "passwords",
		"config": []
	},
...
EOF
	...
	addClientsHysteria "/etc/ss-agent/hysteria/conf/config.json" true
}
...
EOF
		addClients "/etc/ss-agent/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/ss-agent/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
		addClients "/etc/ss-agent/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
	addClients "/etc/ss-agent/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
EOF
		addClients "/etc/ss-agent/xray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/xray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/xray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/xray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
		addClients "/etc/ss-agent/xray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
...
EOF
	addClients "/etc/ss-agent/xray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
}
...
backupNginxConfig() {
	if [[ "$1" == "backup" ]]; then
		cp /etc/nginx/conf.d/alone.conf /etc/ss-agent/alone_backup.conf
		echoContent green " ---> nginx配置文件备份成功"
	fi

	if [[ "$1" == "restoreBackup" ]] && [[ -f "/etc/ss-agent/alone_backup.conf" ]]; then
		cp /etc/ss-agent/alone_backup.conf /etc/nginx/conf.d/alone.conf
		echoContent green " ---> nginx配置文件恢复备份成功"
		rm /etc/ss-agent/alone_backup.conf
	fi
...
unInstall() {
	...
	if [[ -d "/etc/ss-agent/tls" ]] && [[ -n $(find /etc/ss-agent/tls/ -name "*.key") ]] && [[ -n $(find /etc/ss-agent/tls/ -name "*.crt") ]]; then
		...
		if [[ -n $(find /tmp/ss-agent-tls -name '*.key') ]]; then
			echoContent yellow " ---> 备份证书成功，请注意留存。[/tmp/ss-agent-tls]"
		fi
	fi
	...
	echoContent green " ---> 卸载ss-agent脚本完成"
}
...
# 更新脚本
updateV2RayAgent() {
	echoContent skyBlue "\n进度  $1/${totalProgress} : 更新ss-agent脚本"
	...
	local version
	version=$(grep '当前版本:v' "/etc/ss-agent/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/ss-agent" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/mack-a/ss-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		if [[ -n $(ls /etc/ss-agent/subscribe/) ]]; then
			find /etc/ss-agent/subscribe/* | while read -r email; do
				email=$(echo "${email}" | awk -F "[b][e][/]" '{print $2}')

				local base64Result
				base64Result=$(base64 -w 0 "/etc/ss-agent/subscribe/${email}")
...
menu() {
	...
	echoContent red "\n=============================================================="
	echoContent red "                        推广区                      "
	echoContent green "AFF捐赠：https://github.com/mack-a/ss-agent/blob/master/documents/donation_aff.md\n"
